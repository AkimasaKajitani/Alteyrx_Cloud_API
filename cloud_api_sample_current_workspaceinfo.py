...
# 設定ファイル（YAML）の読み込み
with open('cloud_api_sample_current_workspaceinfo_config.yaml', 'r', encoding='utf-8') as f:
...
# 請求情報保存
billinginfo = AYX_CloudAPI.get_current_billing_accounts(AYX_CLOUD_URL, access_token)
...
# ワークスペース情報取得
workspaceinfo = AYX_CloudAPI.get_current_workspace(AYX_CLOUD_URL, access_token)
...
# ユーザー一覧取得
workspace_users = AYX_CloudAPI.get_workspace_users(AYX_CLOUD_URL, access_token, workspaceinfo['id'])

# Fullバージョンはtsvで保存
df_workspace_users_full = pd.DataFrame(workspace_users['data'])
...
print(f"ワークスペース{workspaceinfo['id']}のRole Policyリスト：")
print(df_rolelist)
