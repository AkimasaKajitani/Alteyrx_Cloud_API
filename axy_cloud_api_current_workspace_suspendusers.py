...
with open(SUSPEND_USER_LIST, mode='r', encoding='utf-8') as f:
...
for ws_id, user_ids in grouped_data.items():
    if str(ws_id) == str(current_workspace_id):
        ...
        response = axycloudapi.set_suspend_users(AYX_CLOUD_URL, access_token, current_workspace_id, user_ids)
        print(f"ユーザーサスペンドのレスポンス：")
        print(response)
    else:
